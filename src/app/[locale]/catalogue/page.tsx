          ...
          {COLORS.map((color) => (
            <button key={color} onClick={() => setActiveColor(color)}
              className={cn(
                ...
                activeColor === color
                  ? "border-[#954C2E] text-[#954C2E] bg-[#954C2E]/5 font-medium"
                  : "border-[#E8E5E0] text-[#6B6867] hover:border-[#954C2E]/40"
              )}>
              {color === "all" ? t("all") : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((product) => (
              <Link key={product.id} href={`/${locale}/catalogue/${product.id}`}
                className="group border border-[#E8E5E0] hover:border-[#954C2E]/30 hover:shadow-sm transition-all bg-white">
                {/* Image */}
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-2">
                    <h3 className="text-[#1C1C1E] text-sm font-medium group-hover:text-[#954C2E] transition-colors">
                      {product.name}
                    </h3>
